...
rx = (0.0, 0.0, 0.4)            # 受信点の相対座標（x->y,y->z,z->x）
...
class AnalyticalSolution():
    ...
    def gaussiandoubleprime(self, time):
        ...
        delay = time - self.chi
        return 2 * self.zeta * (2 * self.zeta * delay**2 - 1) * np.exp(-self.zeta * delay**2)

    def Ex(self, time):
        """Calculates Ex component of the electric field at a specific time."""
        t = time - self.tau
        return (self.x * self.z / (4 * np.pi * self.epsilon * self.r**5)) * (3 * self.gaussian(t) + 3 * self.tau * self.gaussianprime(t) + self.tau**2 * self.gaussiandoubleprime(t))
    
    def Ey(self, time):
        """Calculates Ey component of the electric field at a specific time."""
        t = time - self.tau
        return (self.y * self.z / (4 * np.pi * self.epsilon * self.r**5)) * (3 * self.gaussian(t) + 3 * self.tau * self.gaussianprime(t) + self.tau**2 * self.gaussiandoubleprime(t))
    
    def Ez(self, time):
        """Calculates Ez component of the electric field at a specific time."""
        t = time - self.tau
        return (1 / (4 * np.pi * self.epsilon * self.r**5)) * ((2 * self.z**2 - (self.x**2 + self.y**2)) * (self.gaussian(t) + self.tau * self.gaussianprime(t)) - (self.x**2 + self.y**2) * self.tau**2 * self.gaussiandoubleprime(t))
    
    def Hx(self, time):
        """Calculates Hx component of the magnetic field at a specific time."""
        t = time - self.tau
        return - (self.y / (4 * np.pi * self.r**3)) * (self.gaussianprime(t) + self.tau * self.gaussiandoubleprime(t))

    def Hy(self, time):
        """Calculates Hy component of the magnetic field at a specific time."""
        t = time - self.tau
        return (self.x / (4 * np.pi * self.r**3)) * (self.gaussianprime(t) + self.tau * self.gaussiandoubleprime(t))
